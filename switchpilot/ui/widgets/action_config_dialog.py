...
class ActionConfigDialog(QDialog):
        ...
        for idx, action_data in enumerate(self.action_config_list):
            ...
            if params:
                ...
                if 'scene_name' in params:
                    param_summary.append(f"'{params['scene_name']}'")
                ...
                # OBS Specific (or other integrations if added)
                    ...
                    param_summary.append(f"Input: '{params['input_name']}'")

                if 'item_name' in params:
                    param_summary.append(f"Fonte: '{params['item_name']}'")
    ...
    def _fill_param_fields(self, params):
        ...
        if not params:
            ...
            self.save_current_action_button.setEnabled(self.current_editing_action_index != -1
                                                       or (self.integration_combo.currentIndex() > 0 and
                        ...
                        if isinstance(control_widget, QLineEdit):
                            ...
                            if combo_idx != -1:
                                ...
                                if control_widget.isEditable():
                                    ...
                                    if default_mix_idx != -1:
                                        control_widget.setCurrentIndex(default_mix_idx)
    ...
    def get_current_action_params(self):
        ...
        if integration == "OBS Studio":
            ...
            if action_type == "Function (Genérico)":
                ...
                if fn_name_edit:
                    params["function_name"] = fn_name_edit.text().strip()
                ...
                if selected_name_edit:
                    params["selected_name"] = selected_name_edit.text().strip()

                text_value_edit = self.findChild(QLineEdit, "param_text_value")
                if text_value_edit:
                    params["text_value"] = text_value_edit.text().strip()
                ...
                if duration_edit:
                    val = duration_edit.text().strip()
                    if val:
                        params["vmix_duration_for_fade"] = val
